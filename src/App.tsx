    ...
    <div>
      ...
      <div style={{background:"#fff",borderRadius:16,padding:14,marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:3,alignItems:"flex-end",height:60}}>
          {last14.map((l,i)=>{
            const m=MENTAL_OPTIONS.find(o=>o.label===l.mental);
            const h=m?((m.score/3)*50):0;
            return(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                <div style={{width:"100%",height:`${h}px`,background:m?.color||"#eee",borderRadius:"4px 4px 0 0",minHeight:4}}/>
                <div style={{fontSize:8,color:"#ccc",writingMode:"vertical-rl",transform:"rotate(180deg)"}}>{l.date.slice(5)}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{background:"#fff",borderRadius:16,padding:14,marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:3,alignItems:"flex-end",height:60}}>
          {last14.map((l,i)=>{
            const c=CONDITION_OPTIONS.find(o=>o.label===l.condition);
            const h=c?((c.score/5)*50):0;
            return(
              <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                <div style={{width:"100%",height:`${h}px`,background:c?.color||"#eee",borderRadius:"4px 4px 0 0",minHeight:4}}/>
                <div style={{fontSize:8,color:"#ccc",writingMode:"vertical-rl",transform:"rotate(180deg)"}}>{l.date.slice(5)}</div>
              </div>
            );
          })}
      ...
      <div style={{background:"#fff",borderRadius:16,padding:14,marginBottom:10}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:8}}>
          {MENTAL_OPTIONS.map(m=>{
            const cnt=mentalCount[m.label]||0;
            const pct=last14.length>0?Math.round((cnt/last14.length)*100):0;
            return(
              <div key={m.label} style={{flex:1,textAlign:"center"}}>
                <div style={{fontSize:20,marginBottom:4}}>{m.emoji}</div>
                <div style={{height:6,background:"#f0f0f0",borderRadius:3,marginBottom:4,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${pct}%`,background:m.color,borderRadius:3,transition:"width 0.5s"}}/>
                </div>
                <div style={{fontSize:11,color:m.color,fontWeight:"bold"}}>{pct}%</div>
...
export default function App() {
  ...
  const [checkups,setCheckups]=useState<any[]>([INITIAL_CHECKUP]);
  ...
  const handleFoodPhoto=async(e:any)=>{
  ...
  const handleNutritionPhoto=async(e:any)=>{
    const file=e.target.files[0];if(!file)return;
    setAnalyzing(true);
    const b64=await toBase64(file);
  ...
  const addToMyList=()=>{
    if(!myListForm.name)return;
    setMyList(l=>[...l,{...myListForm,cal:Number(myListForm.cal)||0,protein:Number(myListForm.protein)||0,fat:Number(myListForm.fat)||0,carbs:Number(myListForm.carbs)||0,fiber:Number(myListForm.fiber)||0}]);
    setMyListForm({name:"",cal:"",protein:"",fat:"",carbs:"",fiber:""});setShowMyListForm(false);
  };

  const filteredAlcohol=alcoholSearch.length>0?ALCOHOL_DB.filter(a=>a.name.includes(alcoholSearch)).slice(0,6):[];
  const addAlcohol=(a:any,qty:number=1)=>{
  ...
  const alcoholByPeriod=()=>alcoholLogs.reduce((acc:any,l:any)=>{
  ...
  const handleCheckupPhoto=async(e:any)=>{
    const file=e.target.files[0];if(!file)return;
    setCheckupAnalyzing(true);
    const b64=await toBase64(file);
    const result=await analyzeCheckupImage(b64.split(",")[1]);
    if(result)setCheckupForm((f:any)=>({...f,...result}));
      ...
      <div style={{padding:"14px 14px 90px"}}>
        ...
        {tab==="気分"&&(
          <div>
            ...
            {showMoodAnalysis
              ?<MoodAnalysis moodLogs={moodLogs}/>
              :<div>
              ...
              <div style={{...s.card,border:"2px solid #ffe8b0"}}>
                ...
                {(()=>{const key=alcoholView==="week"?thisWeek:alcoholView==="month"?thisMonth:thisYear;const stat=alcoStats[key]||{cal:0,alcohol:0,count:0};return(
                  <div style={{display:"flex",gap:6}}>
                    {[["🍺","杯数",`${stat.count}杯`],["🔥","kcal",`${stat.cal}`],["🌊","純AL",`${stat.alcohol}g`]].map(([ic,lb,val])=>(
                  ...
                  {myList.map((f:any,i:number)=>(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",borderRadius:8,background:"#fff",marginBottom:4}}>
                      <div>
                        ...
                        <div style={{fontSize:10,color:"#bbb"}}>P:{f.protein}g F:{f.fat}g C:{f.carbs}g</div>
                      </div>
        ...
        {tab==="生理"&&(
          <div>
              ...
              return(
                <div style={{background:phase.bg,borderRadius:20,padding:18,marginBottom:14,border:`2px solid ${phase.color}30`}}>
                  ...
                  <div style={{marginBottom:10}}>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{phase.nutrients.map((n:string)=>(<span key={n} style={{background:`${phase.color}20`,color:phase.color,borderRadius:8,padding:"3px 10px",fontSize:12,fontWeight:"bold"}}>{n}</span>))}</div>
                  </div>
                  <div style={{marginBottom:10}}>
                    <div style={{fontSize:12,fontWeight:"bold",color:phase.color,marginBottom:6}}>🍽️ おすすめ食材</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{phase.foods.map((f:string)=>(<span key={f} style={{background:"#fff",border:`1px solid ${phase.color}40`,borderRadius:8,padding:"3px 10px",fontSize:12,color:"#555"}}>{f}</span>))}</div>
                  </div>
                  <div style={{marginBottom:10}}>
                    <div style={{fontSize:12,fontWeight:"bold",color:"#e74c3c",marginBottom:6}}>⚠️ 控えたいもの</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{phase.avoid.map((a:string)=>(<span key={a} style={{background:"#fff0f0",border:"1px solid #ffcccc",borderRadius:8,padding:"3px 10px",fontSize:12,color:"#e74c3c"}}>{a}</span>))}</div>
                  </div>
            ...
            {prediction&&(
              <div style={{background:"linear-gradient(135deg,#ff4d79,#ffb3c6)",borderRadius:20,padding:22,marginBottom:14,textAlign:"center",color:"#fff"}}>
          ...
          <div>
            ...
            {recheckAlerts.length>0&&(
              <div style={{background:"linear-gradient(135deg,#ffe066,#ffb347)",borderRadius:16,padding:14,marginBottom:12,color:"#7a4f00"}}>
                ...
                {recheckAlerts.map((r:any,i:number)=>(<div key={i} style={{fontSize:13,marginBottom:2}}>・{r.item}（{r.due}）</div>))}
              </div>
            )}
            ...
            {showCheckupForm&&(
              <div style={{...s.card,marginBottom:14}}>
                ...
                {[["date","受診日","date"],["place","受診した病院・クリニック名","text"],["height","身長(cm)","number"],["weight","体重(kg)","number"],["bmi","BMI","number"],["sbp","収縮期血圧","number"],["dbp","拡張期血圧","number"],["ast","AST(GOT)","number"],["alt","ALT(GPT)","number"],["gtp","γ-GTP","number"],["alp","ALP","number"],["uricAcid","尿酸","number"],["totalCholesterol","総コレステロール","number"],["hdl","HDL(善玉)","number"],["ldl","LDL(悪玉)","number"],["triglyceride","中性脂肪","number"],["nonHdl","nonHDL","number"],["glucose","空腹時血糖","number"],["hba1c","HbA1c","number"],["creatinine","クレアチニン","number"],["egfr","eGFR","number"],["rbc","赤血球数","number"],["hematocrit","ヘマトクリット","number"],["hemoglobin","ヘモグロビン","number"],["mcv","MCV","number"],["mch","MCH","number"],["mchc","MCHC","number"],["wbc","白血球数","number"],["platelet","血小板数","number"],["memo","メモ・所見","text"]].map(([k,ph,t])=>(
                  <input key={k} type={t} placeholder={ph} value={checkupForm[k]} onChange={(e:any)=>setCheckupForm((f:any)=>({...f,[k]:e.target.value}))} style={s.inp}/>
                ))}
            ...
            {checkups[selectedCheckup]&&(()=>{
              ...
              return(
                <div style={s.card}>
                  ...
                  {[["身体計測",[["身長",c.height,"cm"],["体重",c.weight,"kg"],["BMI",c.bmi,""]]],["血圧",[["収縮期",c.sbp,""],["拡張期",c.dbp,""]]],["肝機能",[["AST",c.ast,""],["ALT",c.alt,""],["γ-GTP",c.gtp,""],["ALP",c.alp,""]]],["尿酸",[["尿酸",c.uricAcid,""]]],["脂質代謝",[["総コレステロール",c.totalCholesterol,""],["HDL",c.hdl,""],["LDL",c.ldl,""],["中性脂肪",c.triglyceride,""],["nonHDL",c.nonHdl,""]]],["糖代謝",[["血糖",c.glucose,""],["HbA1c",c.hba1c,"%"]]],["腎機能",[["クレアチニン",c.creatinine,""],["eGFR",c.egfr,""]]],["血球",[["赤血球",c.rbc,""],["Ht",c.hematocrit,"%"],["Hb",c.hemoglobin,""],["MCV",c.mcv,""],["MCH",c.mch,""],["MCHC",c.mchc,""],["白血球",c.wbc,""],["血小板",c.platelet,""]]]].map(([label,items]:any)=>(
                    <div key={label} style={{marginBottom:10}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>
                        {items.map(([n,v,u]:any)=>v&&(<div key={n} style={{fontSize:12,color:"#555",display:"flex",justifyContent:"space-between",padding:"3px 6px"}}><span style={{color:"#aaa"}}>{n}</span><span style={{fontWeight:"bold",color:"#333"}}>{v}{u}</span></div>))}
                      </div>
                    </div>
                  ))}
                  {c.recheck?.length>0&&(<div style={{marginTop:8,background:"#fffbe6",borderRadius:10,padding:10}}><div style={{fontWeight:"bold",color:"#e6a817",marginBottom:4,fontSize:13}}>⚠️ 再検査</div>{c.recheck.map((r:any,i:number)=>(<div key={i} style={{fontSize:12,color:"#7a5700",marginBottom:2}}>・{r.item}（{r.timing}、{r.due}）</div>))}</div>)}
              ...
              return(
                <div style={{marginBottom:16}}>
                  ...
                  {feedbacks.map((fb:any,i:number)=>(
                    <div key={i} style={{background:fb.type==="praise"?"linear-gradient(135deg,#fff0f4,#ffe8f0)":"linear-gradient(135deg,#f0f8ff,#e8f4ff)",borderRadius:16,padding:"14px 16px",marginBottom:8,borderLeft:`4px solid ${fb.type==="praise"?"#ff4d79":"#60a5fa"}`}}>
